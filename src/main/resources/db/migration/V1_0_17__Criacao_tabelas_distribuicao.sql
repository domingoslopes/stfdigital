...
create table autuacao.distribuicao (seq_distribuicao bigint not null, dsc_justificativa varchar2(500), dat_distribuicao date not null, sig_usuario_distribuicao varchar2(30) not null, seq_peticao bigint not null, seq_processo bigint not null, tip_distribuicao varchar2(10) not null, constraint pk_distribuicao primary key (seq_distribuicao));
...
alter table autuacao.distribuicao add constraint ck_dist_tip_distribuicao check (tip_distribuicao in ('COMUM', 'PREVENCAO'));
...
alter table autuacao.distribuicao_min_impedido add constraint FK_ministro_DIMI foreign key (cod_ministro) references AUTUACAO.ministro;
